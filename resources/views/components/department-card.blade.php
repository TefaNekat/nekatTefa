...
<article class="group flex h-full flex-col rounded-2xl border border-[var(--color-neutral-200)] bg-white p-5 shadow-[var(--shadow-sm)] transition duration-150 hover:-translate-y-0.5 hover:border-[var(--color-primary-500)] hover:shadow-[var(--shadow-md)]">
    <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-[var(--color-primary-100)] text-sm font-extrabold text-[var(--color-primary-600)]">{{ strtoupper(substr($namaJurusan, 0, 2)) }}</div>
    <h3 class="mt-5 font-display text-lg font-bold text-[var(--color-neutral-900)]">{{ $namaJurusan }}</h3>
